...
export class MainComponent implements OnInit {
  ...
  scroll() {
    document.getElementById('mynetwork2').scrollIntoView({ behavior: 'smooth' });
  }
  handleGraphs() {
    ...
    const cities = ['Delhi', 'Mumbai', 'Gujarat', 'Goa', 'Kanpur', 'Jammu', 'Hyderabad', 'Bangalore', 'Gangtok', 'Meghalaya',
      'Chennai', 'Jaipur', 'Surat', 'Noida', 'Raipur', 'Banares'];
    ...
    function createData() {
      V = Math.floor(Math.random() * (cities.length - 6)) + 7; // Ensures V is between 8 and cities.length
      let nodes = [];
      for (let i = 1; i <= V; i++) {
